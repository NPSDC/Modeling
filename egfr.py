...
def calculate_mfs(initial_vals):
	#Raf
...
def calculate_egfr(time, time_index, egf, egf_index):
	egfr = np.linspace(0, egf[egf_index], egf.size)
	time_low = fuzz.zmf(time, 0, egf[egf.size - 1] - egf[egf_index] + egf[1] )
	time_high = fuzz.smf(time, 0, egf[egf.size - 1] - egf[egf_index] + egf[1] )
	egfr_low = fuzz.gaussmf(egfr, 0, egf[egf_index]/20)
	egfr_high = transform_gaussmf(egfr, (egf[egf_index], egf[egf_index]/20), 1)
	a1 = time_low[time_index]
	c1 = np.fmin(a1, egfr_low)
	a2 = time_high[time_index]
	c2 = np.fmin(a2, egfr_high)
	c_com = np.fmax(c1, c2)
...
def rules(input_values, initial_vals, mfs, time_index,):
	y = np.copy(input_values)
	y[1] = calculate_egfr(initial_vals[-1], time_index, initial_vals[0], np.searchsorted(initial_vals[0], input_values[0], 'left') )
	y[2] = calculate_raf((mfs[0], mfs[1]), (mfs[2] , mfs[3]), initial_vals[3], np.searchsorted(initial_vals[2], input_values[1], 'left'))
	return y
...
def main():
	...
	'''for i in xrange(1,size):
		...
		for t in xrange(1,size):
			if t == 0:
				egfr_out[t] = 0
				continue
			egfr_out[t] = calculate_egfr(time, t, egf, i)
		
		if i%10 == 0 and egf[i] == 0.9:
	...
	mfs = calculate_mfs(initial_vals)
	y.resize(1, 3)
